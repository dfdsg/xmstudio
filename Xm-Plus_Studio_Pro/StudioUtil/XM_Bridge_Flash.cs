...
namespace XM_Tek_Studio_Pro.StudioUtil
{
    ...
    class XM_Bridge_Flash 
    {
        public static void InterfaceEnable()
        {
            
            GL600IOCtrl.GpioOE(1, 1, 1, 0, 0, 0, 0); // GPIO1:CS, GPIO2:SK, GPIO3:DI, GPIO4:DO,
        }

        public static void InterfaceSet(int cs, int sk, int di)
        {
            GL600IOCtrl.GpioWR(cs, sk, di, 0, 0, 0, 0);
        }

        public static int Dout()
        {
            int data;
            data = (GL600IOCtrl.GpioRD() & 0x08) >> 3;
            return data;
        }
        ...
        public static int Read(byte addr)
        {
            ...
            XM_Bridge_Flash.InterfaceSet(0, 0, 0); // CS = "L", disable
            for (x = (wrcnt - 1); x >= 0; x--)
            {
                bit = (data >> x) & 0x01;
                XM_Bridge_Flash.InterfaceSet(1, 0, bit);
                XM_Bridge_Flash.InterfaceSet(1, 1, bit);
            }
            XM_Bridge_Flash.InterfaceSet(1, 0, 0);

            rddata = 0;
            for (x = (rdcnt - 1); x >= 0; x--)
            {
                XM_Bridge_Flash.InterfaceSet(1, 1, 0);
                XM_Bridge_Flash.InterfaceSet(1, 0, 0);
                bit = XM_Bridge_Flash.Dout();
                rddata = (bit << x) + rddata;
            }
            XM_Bridge_Flash.InterfaceSet(0, 0, 0);
            return rddata;
        }
